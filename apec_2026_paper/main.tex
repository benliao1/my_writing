...
Figure \ref{fig:scalable_full_schematic} shows the proposed novel scalable dynamic capacitive WPT topology. On the roadway side, an $N$-level stacked-inverter architecture introduced in \cite{2025_Etta_APEC} combines the power from $N$ high-frequency inverters. On the vehicle side, power is split between the $+jX$ and $-jX$ branches. On each branch, an $M$-level stacked-rectifier structure using a series-in parallel-out transformer network splits the power into $M$ high-frequency half-bridge diode rectifiers. The rectified power is then combined in parallel and fed through a single DC-DC converter on each branch. This topology meshes the wide dynamic compensation range of the AVR rectifier with the high-efficiency scalable system proposed in \cite{2025_Etta_APEC}, all with a low component count. Moreover, the design of the matching network and AVR differential impedance $X$ can be co-designed in the same manner as described in \cite{2019_Sinha_Journal} by simply replacing $V_{in}$ and $V_{out}$ with an effective $V_{in,eff} = \kappa N V_{in}$ and $V_{out,eff} = \kappa M V_{out}$, where $\kappa$ is the transformer coupling coefficient. The novel architecture constitutes the first significant contribution of this paper.

\begin{figure*}[!h]
	\centering
	\includegraphics[width=\textwidth]{figures/sim_results_eps.eps}
	\caption{From left to right: inverter output waveforms $v_{inv}$ and $i_{inv}$ in steady-state; AVR branch currents $i_{+jX}$ and $i_{-jX}$ in steady-state; $p_{out}$ and $r_s$ vs. time, showing the AVR rectifier's startup transient under the proposed controller for $t \in [0, 0.01)$, followed by a step change in coupling reactance $\Delta X_s$ at time $t = 0.01$, marked by the red vertical line.}
...
A model is built in Simulink and PLECS to validate the proposed topology and controller for a 6.78 MHz, 12-kW system with $V_{in} = V_{out} = 400$ V. Figure \ref{fig:sim_results} shows steady-state inverter output waveforms and AVR branch currents. It also shows the control objectives $p_{out}$ and $r_s$ being controlled to $P_{out} = 12 \text{ kW}$ and $R_s = 43.2 \text{ }\Omega$, while the coupling reactance is disturbed by an amount $\Delta X_s \equiv \frac{1}{2 \pi f_s}(\frac{1}{C_{s,nom}} - \frac{1}{C_s})$, where $f_s$ is the switching frequency, and $C_{s,nom}$ is the nominal coupling capacitance. The state-space model, full state feedback controller, and simulation validation of the proposed architecture comprise the second significant contribution of this paper.

\section{Hardware Prototype and Experimental Results}
\label{sec:hardware}

The proposed topology and controller are realized on a prototype 6.78 MHz, 12-kW AVR-equipped scalable capacitive WPT system. To achieve this power level, three 4-kW-rated full-bridge inverters and three half-bridge rectifiers in each AVR branch ($N = M = 3$) are paralleled. A four-layer AVR rectifier PCB is designed, along with a separate two-layer control daughterboard. The AVR rectifier PCB is shown with annotations in Figure \ref{fig:power_board}, and the control board PCB is shown with annotations in Figure \ref{fig:control_board}. Critical components and their specifications are listed in Table \ref{tab:components}. 

\begin{table}[h!]
\centering
\begin{tabular}{|c|c|c|} 
 \hline
 \textbf{Component} & \textbf{Part Number} & \textbf{Notes} \\ 
 \hline
 SiC MOSFET & S2M0040120D-1 & 1200 V, 55 A \\ 
 Rectifier Diodes & C4D20120H & 1200 V, 54 A \\
 $L_1$ and $L_2$ Cores & B65982Q0100K097 & TDK 65/60 \\
 Microcontroller & F2800132PTR & TI C2000 \\
 Gate Driver & UCC5350SBDR & TI 8.5/10 A \\
 \hline
\end{tabular}
\caption{Table of critical components on PCBs.}
\label{tab:components}
\end{table}

The DC-DC converters are implemented as four-switch non-inverting buck-boost converters, with each switch implemented as two SiC MOSFETs in parallel. Careful attention was paid to the layout of these parallel SiC devices, to ensure reliable, noise-free, and synchronized switching.

Figure \ref{fig:setup} shows the hardware prototype and experimental setup with annotations labeling important components. The system has been tested up to a power level of 2 kW, and $V_{in} = V_{out} = 150$ V. Annotated waveforms for this scenario are shown in Figure \ref{fig:waveforms}. All three inverter switch nodes achieve soft-switching. Since the system is not misaligned here, the AVR branch currents are almost identical, which is the expected result. 

\begin{figure*}
\centering
\includegraphics[width=0.7\linewidth]{figures/setup_annotate_eps.eps}
\caption{Hardware setup annotated.}
\label{fig:setup}	
\vspace{-0.5cm}
\end{figure*}
...
\begin{tabular}{|c|c|c|c|c|} 
 \hline
 \textbf{Parameter} & \textbf{Value} & & \textbf{Parameter} & \textbf{Value} \\ [0.5ex] 
 \hline
 $L_{pri}$ & 9.3 $\mu$H & & $C_{pri}$ & 58 pF \\
 $L_{sec}$ & 21.1 $\mu$H & & $C_{sec}$ & 24 pF \\
 $X$ & 23.6 $\Omega$ & & $L_1 = L_2$ & 200 $\mu$H \\
 $L_x$ & 0.5 $\mu$H & & $C_x$ & 1.0 nF \\
 $V_{in} = V_{out}$ & 300 V & & $P_{out}$ & 12 kW \\
 \hline
\end{tabular}
\caption{Table of system parameters.}
\label{tab:parameters}
\end{table}

The co-designed matching network values and AVR rectifier differential reactance $X$, as well as other critical system parameters, are presented in Table \ref{tab:parameters}. Note that the inductive reactance $+jX$ is not realized by adding an explicit inductor; rather, it is instead realized by adding a relatively large capacitor in series with the leakage inductance of the transformers. The hardware and experimental validation of the proposed topology are the third significant contribution of this paper.
...
\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{figures/power_board_annotate_eps.eps}
\caption{Annotated power board PCB.}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{figures/control_board_annotate_eps.eps}
\caption{Annotated control board PCB.}
...
\begin{figure*}[!h]
\centering
\includegraphics[width=\linewidth]{figures/waveforms_annotate_eps.eps}
\caption{Annotated system waveforms at 2 kW output power, nominal misalignment condition.}
\label{fig:waveforms}
\vspace{-0.7cm}	
\end{figure*}

This paper presents a novel architecture for scalable dynamic capacitive WPT systems. A state-space model is developed and a full state feedback controller is designed for the proposed topology, which is then validated in simulation. Finally, a 6.78-MHz, 12-kW AVR-equipped scalable capacitive WPT system with three 4-kW-rated full bridge inverters and three half-bridge rectifiers in each AVR branch is built. Its operation is then validated on this hardware. The next goal will be to show the system operating with fully dynamic closed-loop control at kilowatt-scale, and to validate the proposed topology on hardware up to 12 kW.

\appendices
\section{}
\label{app:partials}

This appendix derives the partial derivatives of $p_{out}$ and $r_s$ with respect to the elements of the state vector $\vec{x} = \begin{bmatrix} v_1 & v_2 & v_{out} & i_{L1} & i_{L2} \end{bmatrix}^T$. $p_{out}$ is the output power, and $r_s$ is the real part of the impedance presented by the secondary-side matching network. 
...
Using these quantities, $r_s$ can be written as:
